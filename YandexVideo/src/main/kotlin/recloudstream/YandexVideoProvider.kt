...
class YandexVideoProvider : MainAPI() {
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return doc.select("a[href*='/video/preview/'], a[href*='/video/'], a[href*='yandex.com/video/']")
            .mapNotNull { element ->
                ...
                val title = element.attr("aria-label").ifBlank { element.attr("title") }.ifBlank { element.text().trim() }.ifBlank { "Yandex Video" }
                newMovieSearchResponse(title, full, TvType.Movie) {
                    posterUrl = element.selectFirst("img")?.let { img ->
                        val src = img.attr("src").ifBlank { img.attr("data-src") }
                        src.takeIf { it.isNotBlank() }?.let { absolute(url, it) }
                    }
                }
            }
            .distinctBy { it.url }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val title = doc.selectFirst("meta[property='og:title']")?.attr("content")
            ?: doc.title().ifBlank { "Yandex Video" }
